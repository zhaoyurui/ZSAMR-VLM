...
promptId = '12'
prompt = "Question: How many clusters are arranged in the square? Answer:"  
    ...
    for dataset_path in dataset_paths:
        ...
        for ModalIndex in range(len(Modality)): 
            ...
            saveFilePath = os.path.join(full_save_folder,
                                        "Test_02_Prompt_" + promptId + "_Model_" + 'BLIP2-opt-6.7b' + "_MaxLen_" + str(
            ...
            for ModuIndex in range(len(ModuClass)):
                ...
                for SampleIndex in range(SampleNum):
                    ImagePath = os.path.join(dataset_path, ModalTemp, ModuTemp, "Modal_" + ModalTemp + "_Modu_" + ModalTemp + "_Index_" + str(SampleIndex + 1) +".png")
                    image = Image.open(ImagePath)

                    inputs = processor(images=image, text=prompt, return_tensors="pt").to(device, dtype=torch.float16)  # .to(device, torch.float16)
